...
def get_ffmpeg_path():
    """Return the FFmpeg executable path for packaged and development builds."""
    if hasattr(sys, '_MEIPASS'):
        # When running as a packaged .exe, PyInstaller extracts FFmpeg to a temporary folder
        return os.path.join(sys._MEIPASS, 'ffmpeg.exe')
    return 'ffmpeg' # When running directly from the .py file
...
def execute_render_thread(mode, folder_path, fps, codec, hardware, list_file_path, files):
    ...
    try:
        ...
        if os.path.exists(output_path) and os.path.getsize(output_path) > 1024:
            ...
            app.after(0, lambda: status_label.configure(text="Done!", text_color="#2FA572"))
            app.after(0, lambda: messagebox.showinfo("Done!", f"Processing completed successfully!\nOutput saved to:\n{output_path}"))
        else:
            raise Exception("The output file is empty or invalid.")

    except subprocess.CalledProcessError:
        app.after(0, lambda: messagebox.showerror("Processing Error", f"Open this log file for details:\n{folder_name}_RenderLog.txt"))
        app.after(0, lambda: status_label.configure(text="Error. Please check the render log.", text_color="red"))
    except Exception as e:
        app.after(0, lambda: messagebox.showerror("Technical Error", f"An issue occurred:\n{e}"))
        app.after(0, lambda: status_label.configure(text="Ready", text_color="gray"))
    finally:
        # Stop the loading indicator and re-enable the button
        app.after(0, progress_bar.stop)
        app.after(0, progress_bar.pack_forget) # Hide the loading indicator
...
def run_render():
    ...
    try:
        ...
        messagebox.showerror("Folder Error", f"Unable to read the selected folder:\n{e}")
        return
        
    if not files:
        file_type = "image files" if mode == "image2mp4" else "video files"
        messagebox.showerror("No Files Found", f"No supported {file_type} were found in this folder.")
        return
    ...
    status_label.configure(text="Processing... Please wait.", text_color="#3B8ED0")
    btn_render.configure(state="disabled", text="PROCESSING...") 
    
    # Show and start the loading indicator
...
ctk.CTkLabel(frame_mode, text="Mode:", font=ctk.CTkFont(weight="bold")).pack(side="left", padx=5)
ctk.CTkRadioButton(frame_mode, text="Image Sequence", variable=mode_var, value="image2mp4", cursor="hand2").pack(side="left", padx=10)
ctk.CTkRadioButton(frame_mode, text="Merge Videos", variable=mode_var, value="merge_mp4", cursor="hand2").pack(side="left", padx=10)

frame_folder = ctk.CTkFrame(app, fg_color="transparent")
frame_folder.pack(pady=5, padx=20, fill="x")
folder_entry = ctk.CTkEntry(frame_folder, textvariable=folder_var, width=320, state="readonly", placeholder_text="Folder path...")
folder_entry.pack(side="left", padx=(0, 10))
ctk.CTkButton(frame_folder, text="Select Folder", command=browse_folder, width=90, cursor="hand2").pack(side="left")

frame_hw = ctk.CTkFrame(app, fg_color="transparent")
frame_hw.pack(pady=5)
ctk.CTkLabel(frame_hw, text="Hardware:", font=ctk.CTkFont(weight="bold")).pack(side="left", padx=5)
...
ctk.CTkLabel(frame_codec, text="Codec:", font=ctk.CTkFont(weight="bold")).pack(side="left", padx=5)
ctk.CTkRadioButton(frame_codec, text="H.264 (Standard)", variable=codec_var, value="h264", cursor="hand2").pack(side="left", padx=10)
ctk.CTkRadioButton(frame_codec, text="H.265 (Smaller File)", variable=codec_var, value="hevc", cursor="hand2").pack(side="left", padx=10)

frame_fps = ctk.CTkFrame(app, fg_color="transparent")
frame_fps.pack(pady=5)
ctk.CTkLabel(frame_fps, text="Frame Rate:", font=ctk.CTkFont(weight="bold")).pack(side="left", padx=5)
...
# --- Initialize loading indicator (hidden until processing starts) ---
progress_bar = ctk.CTkProgressBar(app, mode="indeterminate", width=260)

status_label = ctk.CTkLabel(app, text="Ready to render", text_color="gray", font=ctk.CTkFont(size=12, slant="italic"))
